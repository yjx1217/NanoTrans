...
##############################################################
#  script: batch_plot_rna_modification_results.pl
#  author: Jia-Xing Yue (GitHub ID: yjx1217)
#  last edited: 2022.11.02
#  description: plot rna modification results
#  example: perl batch_plot_rna_modification_results.pl  -i Master_Sample_Table.txt -b $batch_id -top_n 20
##############################################################
...
print "\n\n[$local_time] Starting RNA modification plotting for batch $batch_id ..\n";
...
if ($comparison_groups_count <= 1) {
	...
	for (my $j = 1; $j < $comparison_groups_count; $j++) {
	    ...
	    system("Rscript --vanilla $NANOTRANS_HOME/scripts/plot_rna_modification_results.R --input $batch_id.rna_modification.diffmod.table.tidy.txt  --group_a $comparison_groups[$i] --group_b $comparison_groups[$j] --top_n $top_n --prefix $batch_id.rna_modification.diffmod");
	    system("Rscript --vanilla $NANOTRANS_HOME/scripts/plot_rna_modification_results.R --input $batch_id.rna_modification.majority_direction_kmer_diffmod.table.tidy.txt  --group_a $comparison_groups[$i] --group_b $comparison_groups[$j] --top_n $top_n --prefix $batch_id.rna_modification.majority_direction_kmer_diffmod");
...
$local_time = localtime();
print "\n[$local_time] Finishing RNA modification plotting for batch $batch_id.\n";
