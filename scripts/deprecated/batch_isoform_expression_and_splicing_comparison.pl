...
if ($comparison_groups_count <= 1) {
    ...
    for (my $i = 0; $i < $comparison_groups_count - 1; $i++) {
	    ...
	    $between_group_comparison_count++;
			...
			if (! -e "workdir/isoforms_deseq2_${comparison_group_pair_tag}_results_shrinkage.tsv") {
			...
			system("mv isoforms_deseq2_QCplots_${comparison_group_pair_tag}.pdf workdir");
			system("mv    genes_deseq2_QCplots_${comparison_group_pair_tag}.pdf workdir");
			system("mv         isoforms_deseq2_${comparison_group_pair_tag}.tsv workdir");
			system("mv            genes_deseq2_${comparison_group_pair_tag}.tsv workdir");
			system("mv        isoforms_drimseq_${comparison_group_pair_tag}.tsv workdir");
