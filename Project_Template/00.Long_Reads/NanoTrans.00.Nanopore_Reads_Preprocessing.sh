...
raw_fast5_dir="./raw_fast5/$sample_id" # The directory containing the raw nanopore reads before basecalling. Default = "./raw_fast5/$sample_id".
flowcell_version="FLO-MIN106" # The flowcell version of the nanopore run. Default = "FLO-MIN106".
sequencing_kit_version="SQK-RNA001" # The sequencing kit version of the nanopore run. Default = "SQK-RNA001". Note that for new DRS runs, this value is likely to be "SQK-RNA002".
...
u_substitution="false" # the u_substitution option for guppy basecalling (for direct RNA-seq only). Default = "false".
reverse_sequence="true" # the reverse_sequence option for guppy basecalling (for direct RNA-seq only). Default = "true".
...
basecalled_fast5_dir="./basecalled_fast5/$sample_id" # The directory containing the basecalled fast5 reads. This directory will be automatically generated when running basecalling.
...
trim_strategy="rna" # Trimming strategy to apply: 'dna' or 'rna' or 'none' (to disable trimming). Default = "rna".
...
then
	...
	do
		...
		--fastq $fastq_input \
		--N50 \
		-o "./../../../$basecalled_summary_dir/${sample_id}_basecalled_reads_Q${qual}_pass_${b}_NanoPlot_out"
	done
